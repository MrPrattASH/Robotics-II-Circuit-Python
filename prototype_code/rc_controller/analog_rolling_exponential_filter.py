...
def read_analog_ch(
    ...
    """
    r_average = 0
    #TODO - see if this poling window is too great to slow down reliability??
    polling_window = 16
    for i in range(polling_window):
        # start low value
        while not pin.value:
            pass
        # get high value
        ...
        average += end - start
    average = average/polling_window

    # convert time.monotonic into a nice 4 digit duty_cycle value
    in_width = floor(average / 1000)
    # deadpoint centre for joystick
    if in_width > 1400 and in_width < 1520:
        out_value = deadpoint
    else:
        out_value = map_range(in_width, 854, 2056, lower_range_bound, upper_range_bound)
